...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var douban = p.douban ? '<span>Douban</span><a href="http://douban.com/people/' + p.douban + '">    ' +
          p.douban + '</a>' : '';
      
      var github = p.github ? '<span>Github</span><a href="https://github.com/' + p.github + '">    ' + p.github + '</a>' : '';

      var mail = p.mail ? '<span>Mail</span><a href="wuhong@douban.com">    ' + p.mail + '</a>' : '';

      var presenterTitle = [p.name];
      if (p.company) {
        presenterTitle.push(p.company);
      }
      html = presenterTitle.join(' - ') + '<br>'; //+ douban + '<br>' + github + '<br>';

      html2 = [mail, github, douban].join('<br>');
